...
namespace DAILibWV.Frostbite
{
    public class EBXStream
    {
        ...
        public EBXStream(Stream s, ToolStripProgressBar pb = null)
        {
            data = s;
            progress = pb;
            Process();
                    ...
                    switch (State)
                    {
                        case 0://process header
                            ...
                            if (Header.magic != 0x0fb2d1ce ||
                                Header.metaSize + Header.payloadSize != (int)data.Length)
                            {
                                State = -1;//Invalid
                                break;
                            }
                            if (progress != null)
                            {
                                progress.Maximum = Header._arraySectionstart;
                                progress.Minimum = Header.metaSize + Header.stringTableSize;
        ...
        private Type ReadType(Stream s, int idx)
        {
            ...
            for (int i = 0; i < res.Descriptor.fieldCount; i++)
            {
                ...
                if (progress != null && s.Position < Header._arraySectionstart && RefreshCount++ % 10 == 0)
                {
                    progress.Value = (int)s.Position;
                    Application.DoEvents();
                }
            }
